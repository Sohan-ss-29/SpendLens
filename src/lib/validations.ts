...
export const toolInputSchema = z.object({
  ...
  monthlySpend: z.number({ invalid_type_error: 'Enter a valid amount' })
    .min(0, 'Must be 0 or more')
    .max(100000, 'Amount seems too high'),
  seats: z.number({ invalid_type_error: 'Enter a valid number' })
...
export const auditFormSchema = z.object({
  teamSize: z.number({ invalid_type_error: 'Enter your team size' })
    .int()
